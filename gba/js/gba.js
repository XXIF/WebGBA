class GameBoyAdvance {
		...
		if (this.reportFPS) {
			runFunc = function () {
				try {
					...
					start = Date.now();
					var framesToRun = self.speedMultiplier || 1;
					for (var i = 0; i < framesToRun; i++) {
						// 如果不是最后一帧，跳过渲染以提高性能
						if (i < framesToRun - 1) {
							self.video.renderPath.skipFrame = true;
						} else {
							self.video.renderPath.skipFrame = false;
						}
						self.advanceFrame();
					}
					++frames;
	...
	setSpeed(multiplier) {
		...
		if (this.audio) {
			this.audio.speedMultiplier = multiplier;
		}
	}
	setSavedata(data) {
	...
	downloadSavedata() {
		...
		var filename = (this.rom && this.rom.code ? this.rom.code : "game") + ".sav";
		var base64 = this.encodeBase64(sram.view);
